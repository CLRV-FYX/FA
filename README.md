...
```bash
python3 tests/run_tests.py            # 跑全部用例（58 个）
...
- ✅ 端到端可用：**58 个回归用例，56 通过 + 2 跳过（环境依赖），0 失败**；`-O0/-O1/-O2/-O3` 四个优化级别结果完全一致
...
- ✅ `if` / `match` 可以当表达式用：`let y = if c { 1 } else { 2 }`
  （分支类型自动统一，`return` / `panic` 结尾的分支算发散；聚合结果暂不支持）
...
| `if` / `match` 表达式返回结构体、枚举、数组 | 先 `let` 一个变量，在分支里赋值；或返回指针 |
